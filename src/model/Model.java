...
public class Model {
	...
	/*
	 * everytime a entry is inserted,consult databse everytime a utility method
	 * will be called, call load_database first
	 */
		...
		if (q3.hasMoreElements()) {
			...
			if (c != null) {
				...
				if (args[1].charAt(args[1].length() - 2) == '0') {
					...
					System.out.println(
							"ALERT: Over 75% of database records are [HIGH/VERY HIGH] risk for Type 2 Diabetes."
									+ args[1].charAt(args[1].length() - 2));
					Alert alert = new Alert(AlertType.INFORMATION);
			...
			if (c != null) {
				{
					...
					alert.setTitle("Query Response");
					alert.setHeaderText("returned : Max Age ");
					alert.setContentText(c.toString());
	...
	public void stat_user_all(String name) {
		...
		/*
		 * if(data.hasSolution()){ System.out.println("success. name all"); }
		 */
	...
	/*
	 * returns records of patients with family history of diabetes values: 0,1,2
	 */
	...
	/*
	 * returns records based on risk level values :
	 */
	...
	public void stat_age_filter_below(int age) {
		Query data = new Query("stat_age_filter_below",
				new Term[] { (new org.jpl7.Integer((age))), new Variable("Records") });
		while (data.hasNext()) {
